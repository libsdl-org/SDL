...
static SDL_bool has_pos(unsigned int *chmap, unsigned int pos)
{
...
static SDL_bool alsa_chmap_has_duplicate_position(struct ALSA_pcm_cfg_ctx *ctx, unsigned int *pos)
{
    unsigned int chan_idx;
...
static int ALSA_OpenDevice(SDL_AudioDevice *device)
{
    ...
    SDL_free(pcm_str);
    if (status < 0) {
        status = SDL_SetError("ALSA: Couldn't open audio device: %s", ALSA_snd_strerror(status));
        goto err_free_device_hidden;
    }
                            ...
                            snd_pcm_stream_t direction, ALSA_Device **unseen, ALSA_Device **seen)
{
    ...
    SDL_bool recording = direction == SND_PCM_STREAM_CAPTURE ? SDL_TRUE : SDL_FALSE; // used for the unicity of the device
...
#undef NAME_FMT
            if (direction == SND_PCM_STREAM_CAPTURE)
                adev->recording = SDL_TRUE;
            else
                adev->recording = SDL_FALSE;
...
static void ALSA_HotplugIteration(SDL_bool *has_default_output, SDL_bool *has_default_recording)
{
    int r;
    snd_ctl_t *ctl;
...
static void ALSA_DetectDevices(SDL_AudioDevice **default_output, SDL_AudioDevice **default_recording)
{
    ...
    SDL_bool has_default_output = SDL_FALSE, has_default_recording = SDL_FALSE;
    ALSA_HotplugIteration(&has_default_output, &has_default_recording); // run once now before a thread continues to check.
    if (has_default_output) {
        *default_output = SDL_AddAudioDevice(/*recording=*/SDL_FALSE, "ALSA default output device", NULL, (void*)&default_output_handle);
    }
...
static void ALSA_DeinitializeStart(void)
{
    ...
    for (dev = hotplug_devices; dev; dev = next) {
        //SDL_LogInfo(SDL_LOG_CATEGORY_AUDIO, "ALSA: at shutdown, removing %s device '%s'", dev->recording ? "recording" : "output", dev->name);
        next = dev->next;
        SDL_free(dev->name);
        SDL_free(dev);
    }
