...
static char *get_pcm_str(void *handle)
{
    ...
    if (dev->iscapture)
        pcm_str = SDL_getenv("SDL_AUDIO_ALSA_PCM_CAPTURE");
    else
        pcm_str = SDL_getenv("SDL_AUDIO_ALSA_PCM_PLAYBACK");
    if (pcm_str)
...
// XXX: this must stay playback/capture symetric.
...
// This is to handle mostly alsa channel maps with SND_CHMAP_NA position fillers (or weird...).
