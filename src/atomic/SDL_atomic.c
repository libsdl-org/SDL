...
bool SDL_CompareAndSwapAtomicU32(SDL_AtomicU32 *a, uint32_t oldval, uint32_t newval)
{
#ifdef HAVE_MSC_ATOMICS
    SDL_COMPILE_TIME_ASSERT(atomic_cas, sizeof(long) == sizeof(a->value));
    return _InterlockedCompareExchange((long *)&a->value, (long)newval, (long)oldval) == (long)oldval;
...
#elif defined(SDL_PLATFORM_SOLARIS)
    SDL_COMPILE_TIME_ASSERT(atomic_cas, sizeof(uint_t) == sizeof(a->value));
    return ((uint32_t)atomic_cas_uint((volatile uint_t *)&a->value, (uint_t)oldval, (uint_t)newval) == oldval);
#elif defined(EMULATE_CAS)
    bool result = false;
...
#elif defined(SDL_PLATFORM_SOLARIS)
    SDL_COMPILE_TIME_ASSERT(atomic_set, sizeof(uint_t) == sizeof(a->value));
    return (uint32_t)atomic_swap_uint((volatile uint_t *)&a->value, v);
#else
    uint32_t value;
    do {
        value = a->value;
...
uint32_t SDL_GetAtomicU32(SDL_AtomicU32 *a)
{
    ...
    return (uint32_t)_SDL_xadd_watcom((volatile int *)&a->value, 0);
#elif defined(HAVE_GCC_ATOMICS)
    return __sync_or_and_fetch(&a->value, 0);
#elif defined(SDL_PLATFORM_MACOS) // this is deprecated in 10.12 sdk; favor gcc atomics.
    ...
    return (uint32_t)atomic_or_uint_nv((volatile uint_t *)&a->value, 0);
#else
    uint32_t value;
    do {
        value = a->value;
