...
const SDL_SteamVirtualGamepadInfo *SDL_GetJoystickDeviceVirtualGamepadInfo(SDL_JoystickID instance_id)
{
    SDL_JoystickDriver *driver;
    int device_index;
 ...
 */
const char *SDL_GetJoystickDeviceName(SDL_JoystickID instance_id)
{
    SDL_JoystickDriver *driver;
    int device_index;
 ...
 */
const char *SDL_GetJoystickDevicePath(SDL_JoystickID instance_id)
{
    SDL_JoystickDriver *driver;
    int device_index;
 ...
 */
int SDL_GetJoystickDevicePlayerIndex(SDL_JoystickID instance_id)
{
    int player_index;
        ...
        if (SDL_InitSubSystem(SDL_INIT_SENSOR) == 0) {
            ...
            if (sensors) {
                ...
                for (i = 0; sensors[i]; ++i) {
                    ...
                    if (!has_ally_accel && SDL_GetSensorDeviceType(sensor) == SDL_SENSOR_ACCEL) {
                        const char *sensor_name = SDL_GetSensorDeviceName(sensor);
                        if (sensor_name && SDL_strcmp(sensor_name, "Sensor BMI320 Acc") == 0) {
                            has_ally_accel = SDL_TRUE;
                        }
                    }
                    if (!has_ally_gyro && SDL_GetSensorDeviceType(sensor) == SDL_SENSOR_GYRO) {
                        const char *sensor_name = SDL_GetSensorDeviceName(sensor);
...
static void AttemptSensorFusion(SDL_Joystick *joystick, SDL_bool invert_sensors)
{
    ...
    if (sensors) {
        for (i = 0; sensors[i]; ++i) {
            ...
            if (!joystick->accel_sensor && SDL_GetSensorDeviceType(sensor) == SDL_SENSOR_ACCEL) {
            ...
            if (!joystick->gyro_sensor && SDL_GetSensorDeviceType(sensor) == SDL_SENSOR_GYRO) {
                /* Increment the sensor subsystem reference count */
                SDL_InitSubSystem(SDL_INIT_SENSOR);
 ...
 */
SDL_JoystickID SDL_GetJoystickDeviceID(SDL_Joystick *joystick)
{
    SDL_JoystickID retval;
...
const char *SDL_GetJoystickName(SDL_Joystick *joystick)
{
    ...
    SDL_LockJoysticks();
    {
        ...
        info = SDL_GetJoystickDeviceVirtualGamepadInfo(joystick->instance_id);
        if (info) {
            retval = info->name;
        } else {
...
Uint16 SDL_GetJoystickDeviceProductVersion(SDL_JoystickID instance_id)
{
    Uint16 version;
    SDL_JoystickGUID guid = SDL_GetJoystickDeviceGUID(instance_id);
...
SDL_JoystickType SDL_GetJoystickDeviceType(SDL_JoystickID instance_id)
{
