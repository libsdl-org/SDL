...
static bool HIDAPI_DriverXbox360_IsSupportedDevice(SDL_HIDAPI_Device *device, const char *name, SDL_GamepadType type, uint16_t vendor_id, uint16_t product_id, uint16_t version, int interface_number, int interface_class, int interface_subclass, int interface_protocol)
{
...
static bool SetSlotLED(SDL_hid_device *dev, uint8_t slot, bool on)
{
    const bool blink = false;
    uint8_t mode = on ? ((blink ? 0x02 : 0x06) + slot) : 0;
    uint8_t led_packet[] = { 0x01, 0x03, 0x00 };
...
static bool HIDAPI_DriverXbox360_RumbleJoystick(SDL_HIDAPI_Device *device, SDL_Joystick *joystick, uint16_t low_frequency_rumble, uint16_t high_frequency_rumble)
{
#ifdef SDL_PLATFORM_MACOS
    if (SDL_IsJoystickBluetoothXboxOne(device->vendor_id, device->product_id)) {
        uint8_t rumble_packet[] = { 0x03, 0x0F, 0x00, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00 };
         ...
         */
        uint8_t rumble_packet[] = { 'M', 'A', 'G', 'I', 'C', '0', 0x00, 0x04, 0x00, 0x00 };
...
#else
    uint8_t rumble_packet[] = { 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
static bool HIDAPI_DriverXbox360_RumbleJoystickTriggers(SDL_HIDAPI_Device *device, SDL_Joystick *joystick, uint16_t left_rumble, uint16_t right_rumble)
{
    return SDL_Unsupported();
}

static uint32_t HIDAPI_DriverXbox360_GetJoystickCapabilities(SDL_HIDAPI_Device *device, SDL_Joystick *joystick)
{
    SDL_DriverXbox360_Context *ctx = (SDL_DriverXbox360_Context *)device->context;
    uint32_t result = SDL_JOYSTICK_CAP_RUMBLE;
...
static bool HIDAPI_DriverXbox360_SetJoystickLED(SDL_HIDAPI_Device *device, SDL_Joystick *joystick, uint8_t red, uint8_t green, uint8_t blue)
{
    return SDL_Unsupported();
}
...
static void HIDAPI_DriverXbox360_HandleStatePacket(SDL_Joystick *joystick, SDL_DriverXbox360_Context *ctx, uint8_t *data, int size)
{
    int16_t axis;
#ifdef SDL_PLATFORM_MACOS
