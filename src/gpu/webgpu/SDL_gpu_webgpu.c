...
// Bind groups don't get freed unless you free an associated resource.
//  - We should introduce some sort of pruning. (Maybe if it hasn't been bound in N render/compute passes it gets automatically freed?)
...
static bool WebGPUTextureFormatIsBlendable(WGPUTextureFormat format)
{
...
const char *blit2DShaderRGBA32 = R"(
...
const char *blit2DArrayShaderRGBA32 = R"(
//!SDLGPU_COMPAT_F32_UNFILTERABLE
struct SourceRegionBuffer {
    uvLeftTop: vec2<f32>,
    uvDimensions: vec2<f32>,
...
const char *blit3DShaderRGBA32 = R"(
//!SDLGPU_COMPAT_F32_UNFILTERABLE
struct SourceRegionBuffer {
    uvLeftTop: vec2<f32>,
    uvDimensions: vec2<f32>,
...
const char *blitCubeShaderRGBA32 = R"(
//!SDLGPU_COMPAT_F32_UNFILTERABLE
struct SourceRegionBuffer {
    uvLeftTop: vec2<f32>,
    uvDimensions: vec2<f32>,
...
const char *blitCubeArrayShaderRGBA32 = R""(
//!SDLGPU_COMPAT_F32_UNFILTERABLE
struct SourceRegionBuffer {
    uvLeftTop: vec2<f32>,
    uvDimensions: vec2<f32>,
...
static WebGPUTexture *WEBGPU_INTERNAL_CreateTexture(WebGPURenderer *renderer, const SDL_GPUTextureCreateInfo *createInfo)
{
    ...
    desc.label = debugName != NULL ? (WGPUStringView){ debugName, SDL_strlen(debugName) } : WGPU_STRING_VIEW_INIT;
...
static SDL_GPUGraphicsPipeline *WEBGPU_CreateGraphicsPipeline(SDL_GPURenderer *driverData, const SDL_GPUGraphicsPipelineCreateInfo *createInfo)
{
    ...
    for (int i = 0; i < createInfo->target_info.num_color_targets; i++) {
        ...
        if (WebGPUTextureFormatIsBlendable(SDLToWebGPU_TextureFormat[targetDesc->format])) {
    ...
    for (int i = 0; i < createInfo->vertex_input_state.num_vertex_buffers; i++) {
        ...
        for (int j = 0; j < createInfo->vertex_input_state.num_vertex_attributes; j++) {
            if (createInfo->vertex_input_state.vertex_attributes[j].buffer_slot == i) {
                attributeCount++;
                arrayStride += SizeOfSDLVertexFormat[createInfo->vertex_input_state.vertex_attributes[j].format];
            }
        }
    ...
    pipelineDesc.label = debugName != NULL ? (WGPUStringView){ debugName, SDL_strlen(debugName) } : WGPU_STRING_VIEW_INIT;
...
static WebGPUBuffer *WEBGPU_INTERNAL_CreateBuffer(WebGPURenderer *renderer, uint64_t size, SDL_GPUBufferUsageFlags usageFlags, WebGPUBufferType bufferType, const char *debugName)
{
    ...
    WGPUBufferDescriptor desc = {
        .label = debugName != NULL ? (WGPUStringView){ debugName, SDL_strlen(debugName) } : WGPU_STRING_VIEW_INIT,
        .size = size,
        .usage = usages,
...
static SDL_GPUComputePipeline *WEBGPU_CreateComputePipeline(SDL_GPURenderer *device, const SDL_GPUComputePipelineCreateInfo *createInfo)
{
    ...
    shaderDesc.label = WGPU_STRING_VIEW_INIT;
                                    ...
                                    const SDL_GPUStorageBufferReadWriteBinding *storageBufferBindings, Uint32 numStorageBufferBindings)
{
    ...
    passDesc.label = WGPU_STRING_VIEW_INIT;
...
static void WEBGPU_DownloadFromTexture(SDL_GPUCommandBuffer *commandBuffer, const SDL_GPUTextureRegion *source, const SDL_GPUTextureTransferInfo *destination)
{
    ...
    destinationInfo.layout = (WGPUTexelCopyBufferLayout){
        .bytesPerRow = BytesPerRow(source->w, ((WebGPUTextureContainer *)source->texture)->header.info.format),
        .offset = 0,
    };
...
static void WEBGPU_Blit(SDL_GPUCommandBuffer *commandBuffer, const SDL_GPUBlitInfo *info)
{
    ...
    if (((WebGPUTextureContainer *)info->source.texture)->header.info.format == SDL_GPU_TEXTUREFORMAT_R32G32B32A32_FLOAT) {
        SDL_GPU_BlitCommon(
            commandBuffer,
            info,
