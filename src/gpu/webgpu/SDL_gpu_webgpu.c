...
typedef struct WebGPUCommandBuffer
{
    ...
    // The command buffer for the copy pass.
    WGPUCommandBuffer copyCommandBuffer;
    ...
    WebGPUSubmittedCommandBuffer *submittedForCopy;

    // These'll run right before the command buffer is submitted.
    WebGPUQueuedUniformDataUpload *queuedUniformUploads;

    uint32_t numQueuedUniformUploads;
    uint32_t queuedUniformUploadCapacity;
    ...
    struct vertexStageBinds
    {
        ...
        uint32_t currentUniformWriteOffsets[4];
        uint32_t currentUniformReadOffsets[4];
...
static void
WEBGPU_INTERNAL_SubmittedCommandBufferCallback(WGPUQueueWorkDoneStatus status, WGPUStringView message, void *_cmdBuf, void *unused)
{
    // FIXME: There are no references to any submitted command buffers aside from this callback! Bad! No!
    WebGPUSubmittedCommandBuffer *cmdBuf = _cmdBuf;

    for (int i = 0; i < cmdBuf->usedBufferCount; i++) {
        // FIXME: This removes all references, not just the references for this command buffer.
        SDL_SetAtomicInt(&cmdBuf->usedBuffers[i]->referenceCount, 0);
    }

    for (int i = 0; i < cmdBuf->usedTextureCount; i++) {
        SDL_SetAtomicInt(&cmdBuf->usedTextures[i]->referenceCount, 0);
    }

    SDL_free(cmdBuf->usedBuffers);
    SDL_free(cmdBuf->usedTextures);
    SDL_free(cmdBuf);
}
...
static void
WEBGPU_INTERNAL_UncapturedErrorCallback(WGPUDevice const *device, WGPUErrorType type, WGPUStringView message, void *debugMode, void *unused)
{
    if (debugMode) {
    ...
    SDL_GPUCommandBuffer *commandBuffer)
{
    ...
    cmdBuf->copyEncoder = wgpuDeviceCreateCommandEncoder(cmdBuf->device, NULL);
}

static void WEBGPU_EndCopyPass(SDL_GPUCommandBuffer *copyPass)
{
    ((WebGPUCommandBuffer *)copyPass)->copyCommandBuffer = wgpuCommandEncoderFinish((((WebGPUCommandBuffer *)copyPass)->copyEncoder), NULL);
}
...
static SDL_GPUCommandBuffer *WEBGPU_AcquireCommandBuffer(SDL_GPURenderer *device)
{
    ...
    // IMPORTANT: These will be freed by the callback. Is it a shit way of doing it? Yes.
    // Do I care right now? Not really.
    // I just want cycling working first, then we can take care of the edgecases.
    wrapper->submitted = SDL_calloc(1, sizeof(*wrapper->submitted));
    wrapper->submittedForCopy = SDL_calloc(1, sizeof(*wrapper->submittedForCopy));
...
static void WEBGPU_INTERNAL_EnumerateBindGroupsCallback(void *userdata, SDL_PropertiesID props, const char *name)
{
    ...
    if (bindGroup != NULL) {
        if ((renderer->numSubmissions + 1) - bindGroup->lastUsedAtSubmission > renderer->bindGroupsExpireAfter || renderer->destroyingSelf) {
            WEBGPU_INTERNAL_InvalidateBindGroup(renderer, SDL_strdup(name));
        }
    }
}

// This function will automatically invalidate all bind groups if destroyingSelf is TRUE.
static void WEBGPU_INTERNAL_ReleaseInvalidBindGroups(WebGPURenderer *renderer)
{
    if (renderer->bindGroupsExpireAfter >= 0) {
        SDL_EnumerateProperties(renderer->bindGroups, WEBGPU_INTERNAL_EnumerateBindGroupsCallback, renderer);
    }
...
static char *WEBGPU_INTERNAL_GenerateBindGroupIdentifier(WebGPUCommandBuffer *cmdBuf, WebGPUBindGroupType desiredGroup)
{
    ...
    case WEBGPU_BINDGROUP_VERTEXUNIFORMS:
    {
        ...
        for (int i = 0; i < cmdBuf->vertexStageBinds.numUniformBuffers; i++) {
            SDL_memcpy(identifier + offset, cmdBuf->renderer->uniformBuffers[cmdBuf->vertexStageBinds.uniformBuffers[i].slot]->activeBuffer->resourceIdentifier, 8);
            identifier[offset + 8] = '-';
            offset += 9;
        }
        break;
    ...
    case WEBGPU_BINDGROUP_FRAGMENTUNIFORMS:
    {
        ...
        for (int i = 0; i < cmdBuf->fragmentStageBinds.numUniformBuffers; i++) {
            SDL_memcpy(identifier + offset, cmdBuf->renderer->uniformBuffers[3 + cmdBuf->fragmentStageBinds.uniformBuffers[i].slot]->activeBuffer->resourceIdentifier, 8);
            identifier[offset + 8] = '-';
            offset += 9;
        }
        break;
    ...
    case WEBGPU_BINDGROUP_COMPUTEUNIFORMS:
    {
        ...
        for (int i = 0; i < cmdBuf->computeStageBinds.numUniformBuffers; i++) {
            SDL_memcpy(identifier + offset, cmdBuf->renderer->uniformBuffers[7 + cmdBuf->computeStageBinds.uniformBuffers[i].slot]->activeBuffer->resourceIdentifier, 8);
            identifier[offset + 8] = '-';
            offset += 9;
        }
        break;
...
static WebGPUBindGroup *WEBGPU_INTERNAL_CreateBindGroup(WebGPUCommandBuffer *cmdBuf, WebGPUBindGroupType type, char *identifier)
{
    ...
    case WEBGPU_BINDGROUP_VERTEXUNIFORMS:
    {
        desc.entryCount = (size_t)(cmdBuf->vertexStageBinds.numUniformBuffers);
        ...
        for (offset = 0; offset < desc.entryCount; offset++) {
            entries[offset].buffer = cmdBuf->renderer->uniformBuffers[cmdBuf->vertexStageBinds.uniformBuffers[offset].slot]->activeBuffer->buffer;
            entries[offset].binding = cmdBuf->vertexStageBinds.uniformBuffers[offset].slot;
            entries[offset].size = cmdBuf->vertexStageBinds.uniformBuffers[offset].length;
        }
    ...
    case WEBGPU_BINDGROUP_FRAGMENTUNIFORMS:
    {
        desc.entryCount = (size_t)(cmdBuf->fragmentStageBinds.numUniformBuffers);
        desc.layout = cmdBuf->boundGraphicsPipeline->fragmentBindGroupLayouts->uniformBindGroupLayout;

        entries = SDL_calloc(desc.entryCount, sizeof(*entries));

        for (offset = 0; offset < desc.entryCount; offset++) {
            entries[offset].buffer = cmdBuf->renderer->uniformBuffers[3 + cmdBuf->fragmentStageBinds.uniformBuffers[offset].slot]->activeBuffer->buffer;
            entries[offset].binding = cmdBuf->fragmentStageBinds.uniformBuffers[offset].slot;
            entries[offset].size = cmdBuf->fragmentStageBinds.uniformBuffers[offset].length;
        }
    ...
    case WEBGPU_BINDGROUP_COMPUTEUNIFORMS:
        desc.entryCount = (size_t)(cmdBuf->computeStageBinds.numUniformBuffers);
        desc.layout = cmdBuf->boundComputePipeline->bindGroupLayouts->uniformBindGroupLayout;

        entries = SDL_calloc(desc.entryCount, sizeof(*entries));

        for (offset = 0; offset < desc.entryCount; offset++) {
            entries[offset].buffer = cmdBuf->renderer->uniformBuffers[7 + cmdBuf->computeStageBinds.uniformBuffers[offset].slot]->activeBuffer->buffer;
            entries[offset].binding = cmdBuf->computeStageBinds.uniformBuffers[offset].slot;
            entries[offset].size = cmdBuf->computeStageBinds.uniformBuffers[offset].length;
        }
        break;
    }
...
static void WEBGPU_INTERNAL_ReleaseTextureView(WebGPURenderer *renderer, WebGPUTextureView *view)
{
    ...
    SDL_free(view->resourceIdentifier);
    SDL_free(view);
}
...
static void WEBGPU_GenerateMipmaps(SDL_GPUCommandBuffer *commandBuffer, SDL_GPUTexture *_texture)
{
    ...
    for (int i = 0; i < texture->mipLevelViewCount; i++) {
        wgpuCommandEncoderCopyTextureToTexture(cmdBuf->copyEncoder,
...
static void WEBGPU_CopyBufferToBuffer(SDL_GPUCommandBuffer *copyPass, const SDL_GPUBufferLocation *source, const SDL_GPUBufferLocation *destination, uint32_t size, bool cycle)
{
    ...
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->submittedForCopy->usedBuffers, cmdBuf->submittedForCopy->usedBufferCapacity, cmdBuf->submittedForCopy->usedBufferCount, WebGPUBuffer *, ((WebGPUBufferContainer *)source->buffer)->activeBuffer);
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->submittedForCopy->usedBuffers, cmdBuf->submittedForCopy->usedBufferCapacity, cmdBuf->submittedForCopy->usedBufferCount, WebGPUBuffer *, ((WebGPUBufferContainer *)destination->buffer)->activeBuffer);

    WEBGPU_INTERNAL_CopyBufferToBuffer(((WebGPUCommandBuffer *)copyPass)->copyEncoder, (WebGPUBufferContainer *)source->buffer, source->offset, (WebGPUBufferContainer *)destination->buffer, destination->offset, size);
}
...
static void WEBGPU_CopyTextureToTexture(SDL_GPUCommandBuffer *copyPass, const SDL_GPUTextureLocation *source, const SDL_GPUTextureLocation *destination, Uint32 w, Uint32 h, Uint32 d, bool cycle)
{
    ...
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->submittedForCopy->usedTextures, cmdBuf->submittedForCopy->usedTextureCapacity,
                                           cmdBuf->submittedForCopy->usedTextureCount, WebGPUTexture *, ((WebGPUTextureContainer *)source->texture)->activeTexture);
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->submittedForCopy->usedTextures, cmdBuf->submittedForCopy->usedTextureCapacity,
                                           cmdBuf->submittedForCopy->usedTextureCount, WebGPUTexture *, ((WebGPUTextureContainer *)destination->texture)->activeTexture);

    wgpuCommandEncoderCopyTextureToTexture(cmdBuf->copyEncoder, &sourceInfo, &destInfo, &(WGPUExtent3D){ w, h, d });
}
...
static void WEBGPU_UploadToBuffer(SDL_GPUCommandBuffer *copyPass, const SDL_GPUTransferBufferLocation *source, const SDL_GPUBufferRegion *destination, bool cycle)
{
    ...
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->submittedForCopy->usedBuffers, cmdBuf->submittedForCopy->usedBufferCapacity, cmdBuf->submittedForCopy->usedBufferCount, WebGPUBuffer *, ((WebGPUBufferContainer *)source->transfer_buffer)->activeBuffer);
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->submittedForCopy->usedBuffers, cmdBuf->submittedForCopy->usedBufferCapacity, cmdBuf->submittedForCopy->usedBufferCount, WebGPUBuffer *, ((WebGPUBufferContainer *)destination->buffer)->activeBuffer);

    WEBGPU_INTERNAL_CopyBufferToBuffer(((WebGPUCommandBuffer *)copyPass)->copyEncoder, (WebGPUBufferContainer *)source->transfer_buffer, source->offset, (WebGPUBufferContainer *)destination->buffer, destination->offset, destination->size);
}
...
static void WEBGPU_UploadToTexture(SDL_GPUCommandBuffer *copyPass, const SDL_GPUTextureTransferInfo *source, const SDL_GPUTextureRegion *destination, bool cycle)
{
    ...
    if (paddedBytesPerRow != bytesPerRow) {
        ...
        for (int i = 0; i < rowsPerLayer; i++) {
            wgpuCommandEncoderCopyBufferToBuffer(cmdBuf->copyEncoder, userSourceBuffer->buffer, source->offset + i * bytesPerRow,
        ...
        WEBGPU_INTERNAL_ReleaseBuffer(cmdBuf->renderer, babysittingSourceBuffer);
    }
    ...
    if (!hadToPad) {
        ...
        WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->submittedForCopy->usedBuffers, cmdBuf->submittedForCopy->usedBufferCapacity,
                                               cmdBuf->submittedForCopy->usedBufferCount, WebGPUBuffer *, finalSourceBuffer);
    }

    SDL_AtomicIncRef(&((WebGPUTextureContainer *)destination->texture)->activeTexture->referenceCount);
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->submittedForCopy->usedTextures, cmdBuf->submittedForCopy->usedTextureCapacity,
                                           cmdBuf->submittedForCopy->usedTextureCount, WebGPUTexture *,
                                           ((WebGPUTextureContainer *)destination->texture)->activeTexture);

    wgpuCommandEncoderCopyBufferToTexture(cmdBuf->copyEncoder, &sourceInfo, &destInfo, &(WGPUExtent3D){ destination->w, destination->h, destination->d });
}
...
static void WEBGPU_PushVertexUniformData(SDL_GPUCommandBuffer *commandBuffer, uint32_t slotIndex, const void *data, uint32_t length)
{
    ...
    WebGPUQueuedResourceBindUniformBuffer bind = {
    ...
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->vertexStageBinds.uniformBuffers, cmdBuf->vertexStageBinds.uniformBufferArraySize,
                                           cmdBuf->vertexStageBinds.numUniformBuffers, WebGPUQueuedResourceBindUniformBuffer, bind);
...
static void WEBGPU_PushFragmentUniformData(SDL_GPUCommandBuffer *commandBuffer, uint32_t slotIndex, const void *data, uint32_t length)
{
    ...
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->fragmentStageBinds.uniformBuffers, cmdBuf->fragmentStageBinds.uniformBufferArraySize,
                                           cmdBuf->fragmentStageBinds.numUniformBuffers, WebGPUQueuedResourceBindUniformBuffer, bind);
...
static void WEBGPU_PushComputeUniformData(SDL_GPUCommandBuffer *commandBuffer, uint32_t slotIndex, const void *data, uint32_t length)
{
    ...
    WEBGPU_INTERNAL_InsertElementIntoArray(cmdBuf->computeStageBinds.uniformBuffers, cmdBuf->computeStageBinds.uniformBufferArraySize,
                                           cmdBuf->computeStageBinds.numUniformBuffers, WebGPUQueuedResourceBindUniformBuffer, bind);
...
static void WEBGPU_INTERNAL_BindQueuedGraphicsResources(WebGPUCommandBuffer *cmdBuf)
{
    ...
    for (int i = 0; i < 4; i++) {
        if (bindGroups[i] != NULL) {
            if (i == 1) {
                // vert uniform
                wgpuRenderPassEncoderSetBindGroup(cmdBuf->renderPassEncoder, i, bindGroups[i]->bindGroup,
                                                  cmdBuf->vertexStageBinds.numUniformBuffers, cmdBuf->vertexStageBinds.currentUniformReadOffsets);
            } else if (i == 3) {
                // frag uniform
                wgpuRenderPassEncoderSetBindGroup(cmdBuf->renderPassEncoder, i, bindGroups[i]->bindGroup,
                                                  cmdBuf->fragmentStageBinds.numUniformBuffers, cmdBuf->fragmentStageBinds.currentUniformReadOffsets);
            } else {
    ...
    // HACK: Wow, I hate this!
    // This is here to prevent binding prior uniform buffers.
    // Gross hack I know. I hate myself just as much (if not more) than you do.
    cmdBuf->vertexStageBinds.numUniformBuffers = 0;
    cmdBuf->fragmentStageBinds.numUniformBuffers = 0;
...
static void WEBGPU_INTERNAL_BindQueuedComputeResources(WebGPUCommandBuffer *cmdBuf)
{
    ...
    for (int i = 0; i < 3; i++) {
        if (bindGroups[i] != NULL) {
            if (i == 2) {
                wgpuComputePassEncoderSetBindGroup(cmdBuf->computePassEncoder, i, bindGroups[i]->bindGroup,
                                                   cmdBuf->computeStageBinds.numUniformBuffers, cmdBuf->computeStageBinds.currentUniformReadOffsets);
            } else {
    ...
    cmdBuf->computeStageBinds.numUniformBuffers = 0;
...
static bool WEBGPU_Submit(SDL_GPUCommandBuffer *commandBuffer)
{
    ...
    if (wrapper->copyCommandBuffer != NULL) {
        wgpuQueueOnSubmittedWorkDone(wrapper->queue, (WGPUQueueWorkDoneCallbackInfo){
                                                         .callback = WEBGPU_INTERNAL_SubmittedCommandBufferCallback,
                                                         .mode = WGPUCallbackMode_AllowSpontaneous,
                                                         .nextInChain = NULL,
                                                         .userdata1 = wrapper->submittedForCopy,
                                                         .userdata2 = NULL,
                                                     });

        wgpuQueueSubmit(wrapper->queue, 1, &wrapper->copyCommandBuffer);
    }
    ...
    wgpuQueueOnSubmittedWorkDone(wrapper->queue, (WGPUQueueWorkDoneCallbackInfo){
                                                     ...
                                                     .mode = WGPUCallbackMode_AllowSpontaneous,
                                                     .nextInChain = NULL,
                                                     // Submitted isn't freed alongside the rest of the command buffer.
                                                     .userdata1 = wrapper->submitted,
                                                     .userdata2 = NULL,
                                                 });
    ...
    if (wrapper->copyEncoder != NULL) {
        wgpuCommandEncoderRelease(wrapper->copyEncoder);
    }
    if (wrapper->copyCommandBuffer != NULL) {
        wgpuCommandBufferRelease(wrapper->copyCommandBuffer);
    }
...
static void WEBGPU_DestroyDevice(SDL_GPUDevice *device)
{
    ...
    WEBGPU_INTERNAL_ReleaseInvalidBindGroups(renderer);
...
static void WEBGPU_DownloadFromTexture(SDL_GPUCommandBuffer *commandBuffer, const SDL_GPUTextureRegion *source, const SDL_GPUTextureTransferInfo *destination)
{
    ...
    wgpuCommandEncoderCopyTextureToBuffer(((WebGPUCommandBuffer *)commandBuffer)->copyEncoder, &sourceInfo, &destinationInfo, &(WGPUExtent3D){ source->w, source->h, source->layer });
}
...
{
    WebGPUCommandBuffer *cmdBuf = (WebGPUCommandBuffer *)commandBuffer;
    WEBGPU_INTERNAL_CopyBufferToBuffer(cmdBuf->copyEncoder, (WebGPUBufferContainer *)source->buffer, source->offset,
