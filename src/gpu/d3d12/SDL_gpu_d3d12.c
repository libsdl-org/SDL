...
static bool D3D12_INTERNAL_GetAdapterByLuid(LUID luid, IDXGIFactory1 *factory, IDXGIAdapter1 **outAdapter)
{
    ...
    for (Uint32 adapterIndex = 0;; adapterIndex++) {
        ...
        if (FAILED(res)) {
            SDL_LogError(SDL_LOG_CATEGORY_GPU, "Failed to get an adapter when iterating, i: %d, res: %" SDL_PRIdHRESULT, adapterIndex, res);
            return false;
        }
        ...
        if (FAILED(res)) {
            IDXGIAdapter1_Release(adapter);
            SDL_LogError(SDL_LOG_CATEGORY_GPU, "Failed to get description of adapter, i: %d, res %" SDL_PRIdHRESULT, adapterIndex, res);
            return false;
        }
