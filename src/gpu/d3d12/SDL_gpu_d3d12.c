...
static bool D3D12_INTERNAL_ConvertVertexInputState(SDL_GPUVertexInputState vertexInputState, D3D12_INPUT_ELEMENT_DESC *desc, const char *semantic)
{
    ...
    for (Uint32 i = 0; i < vertexInputState.num_vertex_attributes; i += 1) {
        ...
        desc[i].InputSlot = attribute.bindingIndex;
        desc[i].AlignedByteOffset = attribute.offset;
        desc[i].InputSlotClass = SDLToD3D12_InputRate[vertexInputState.vertex_bindings[attribute.bindingIndex].input_rate];
        desc[i].InstanceDataStepRate = (vertexInputState.vertex_bindings[attribute.bindingIndex].input_rate == SDL_GPU_VERTEXINPUTRATE_INSTANCE) ? vertexInputState.vertex_bindings[attribute.bindingIndex].instance_step_rate : 0;
    }
