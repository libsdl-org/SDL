...
static void *SDL_HIDAPI_HapticDriverLg4ff_Open(SDL_Joystick *joystick)
{
    ...
    SDL_snprintf(ctx->thread_name_buf, sizeof(ctx->thread_name_buf), "SDL_hidapihaptic_lg4ff %u %04x:%04x", SDL_GetJoystickID(joystick), (Uint32)USB_VENDOR_ID_LOGITECH, ctx->product_id);
