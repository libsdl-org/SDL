...
void SDL_SetKeymapEntry(SDL_Keymap *keymap, SDL_Scancode scancode, SDL_Keycode keycode)
{
    SDL_Keycode existing_keycode;
    if (!keymap) {
        return;
    }
...
SDL_Keycode SDL_GetKeymapKeycode(SDL_Keymap *keymap, SDL_Scancode scancode)
{
    SDL_Keycode keycode;
    if (keymap) {
        keycode = keymap->scancode_to_keycode[scancode];
        if (keycode == -1) {
            keycode = SDL_GetDefaultKeyFromScancode(scancode);
        }
    } else {
        keycode = SDL_GetDefaultKeyFromScancode(scancode);
    }
...
SDL_Keycode SDL_GetKeyFromName(const char *name)
{
    ...
    if (key != SDLK_UNKNOWN) {
        if (uppercase) {
            ...
            SDL_Scancode scancode = SDL_GetKeymapScancode(keymap, key);
            if (scancode != SDL_SCANCODE_UNKNOWN) {
                key = SDL_GetKeymapKeycode(keymap, scancode);
            }
        }
        return key;
    }

    return SDL_GetKeyFromScancode(SDL_GetScancodeFromName(name), SDL_FALSE);
}
