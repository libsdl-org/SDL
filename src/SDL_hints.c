...
static const char* GetHintEnvVar(const char *name)
{
    const char *ret = SDL_getenv(name);
    if (ret == NULL && name != NULL && *name != '\0') {
        ...
        if (SDL_strcmp(name, SDL_HINT_VIDEO_DRIVER) == 0) {
            ret = SDL_getenv("SDL_VIDEODRIVER");
        } else if(SDL_strcmp(name, SDL_HINT_AUDIO_DRIVER) == 0) {
            ret = SDL_getenv("SDL_AUDIODRIVER");
        }
    }
    return ret;
}
...
bool SDL_SetHintWithPriority(const char *name, const char *value, SDL_HintPriority priority)
{
    ...
    const char *env = GetHintEnvVar(name);
...
const char *SDL_GetHint(const char *name)
{
    ...
    const char *result = GetHintEnvVar(name);
