...
int SDL_SW_FillTriangle(SDL_Surface *dst, const SDL_Point points[3], SDL_BlendMode blend, Uint32 color)
{
    int ret = 0;
    int dst_locked = 0;

    const SDL_Point *d0 = &points[0];
    const SDL_Point *d1 = &points[1];
    const SDL_Point *d2 = &points[2];
...
int SDL_SW_BlitTriangle(SDL_Surface *src, SDL_Surface *dst, const SDL_Point verts[6])
{
    int ret = 0;
    int src_locked = 0;
    int dst_locked = 0;
    ...
    const SDL_Point *s0 = &verts[0];
    const SDL_Point *d0 = &verts[1];

    const SDL_Point *s1 = &verts[2];
    const SDL_Point *d1 = &verts[3];

    const SDL_Point *s2 = &verts[4];
    const SDL_Point *d2 = &verts[5];
    ...
    has_colormod = r != 255 || g != 255 || b != 255;
    ...
    if (blend != SDL_BLENDMODE_NONE || src->format->format != dst->format->format || has_colormod) {
