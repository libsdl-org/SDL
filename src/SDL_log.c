...
void SDL_LogOutputRaw(void *userdata, int category, SDL_LogPriority priority,
                      const char *user_message, const char *processed_message)
{
    ...
    // Note: One can't call SDL_SetError here, since that function itself logs.
    {
        ...
        if (consoleAttached == CONSOLE_ATTACHED_CONSOLE) {
            ...
            if (!WriteFile(stderrHandle, processed_message, (DWORD)SDL_strlen(processed_message), &charsWritten, NULL)) {
