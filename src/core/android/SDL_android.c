    ...
    while (nb_cmd--) {
        ...
        switch (cmd) {
            ...
            case RPC_cmd_onNativePinchStart:
                {
                    RPC_Get(onNativePinchStart);
                    SDL_SendPinch(SDL_EVENT_PINCH_BEGIN, 0, window, 0, data.span_x, data.span_y, data.focus_x, data.focus_y);
                }
                break;
            ...
            case RPC_cmd_onNativePinchUpdate:
                {
                    ...
                    SDL_SendPinch(SDL_EVENT_PINCH_UPDATE, 0, window, data.scale, data.span_x, data.span_y, data.focus_x, data.focus_y);
                }
                break;
            ...
            case RPC_cmd_nativeAddAudioDevice:
                {
                    ...
                    if (SDL_GetCurrentAudioDriver() != NULL) {
                        ...
                        if (!SDL_FindPhysicalAudioDeviceByHandle(handle)) {
                            SDL_AddAudioDevice(data.recording, SDL_strdup(data.name), NULL, NULL, handle);
                        }
                    }
#endif
                    SDL_free(data.name);
...
bool Android_JNI_ReadInt(const char *name, int *val)
{
    ...
    clazz = (*env)->FindClass(env, "org/libsdl/app/SDLActivity");
    if (!val || !clazz) {
        goto error;
    }

    fieldID = (*env)->GetStaticFieldID(env, clazz, name, "I");
    if (!fieldID) {
        goto error;
    }
...
bool Android_JNI_ReadFloat(const char *name, float *val)
{
    ...
    fieldID = (*env)->GetStaticFieldID(env, clazz, name, "F");
    if (!fieldID) {
        goto error;
    }
...
bool Android_JNI_ReadBoolean(const char *name, bool *val)
{
    JNIEnv *env = Android_JNI_GetEnv();
    jclass clazz;
    jfieldID fieldID;
    ...
    fieldID = (*env)->GetStaticFieldID(env, clazz, name, "Z");
    if (!fieldID) {
        goto error;
    }

    *val = (*env)->GetStaticBooleanField(env, clazz, fieldID);
    return true;

error:
    __android_log_print(ANDROID_LOG_ERROR, "SDL", "Failed to find 'bool' parameter '%s'", name);
    return false;
}
