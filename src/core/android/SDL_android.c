...
typedef struct {
    ...
    float focus_x;
    float focus_y;
} RPC_data_onNativePinchEnd_t;

JNIEXPORT void JNICALL SDL_JAVA_INTERFACE(onNativePinchEnd)(
    JNIEnv *env, jclass jcls, jfloat span_x, jfloat span_y, jfloat focus_x, jfloat focus_y)
{
    RPC_Prepare(onNativePinchEnd);
    RPC_Add(span_x);
    RPC_Add(span_y);
    RPC_Add(focus_x);
    RPC_Add(focus_y);
    ...
    while (nb_cmd--) {
        ...
        switch (cmd) {
            ...
            case RPC_cmd_onNativePinchEnd:
                {
                    RPC_Get(onNativePinchEnd);
                    SDL_SendPinch(SDL_EVENT_PINCH_END, 0, window, 0, data.span_x, data.span_y, data.focus_x, data.focus_y);
                }
                break;
