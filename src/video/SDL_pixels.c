...
const SDL_PixelFormatDetails *SDL_GetPixelFormatDetails(SDL_PixelFormat format)
{
    ...
    if (SDL_ShouldInit(&SDL_format_details_init)) {
        SDL_format_details = SDL_CreateHashTable(0, SDL_HashID, SDL_KeyMatchID, SDL_DestroyHashValue, NULL);
...
bool SDL_MapSurface(SDL_Surface *src, SDL_Surface *dst)
{
    ...
    if (SDL_ISPIXELFORMAT_INDEXED(srcfmt->format)) {
        ...
        if (SDL_ISPIXELFORMAT_INDEXED(dstfmt->format)) {
            // BitField --> Palette
            map->info.palette_map = SDL_CreateHashTable(0, SDL_HashID, SDL_KeyMatchID, NULL, NULL);
        } else {
            // BitField --> BitField
            if (srcfmt == dstfmt) {
                map->identity = 1;
