...
static const uint8_t GAMEINPUT_button_map[MAX_GAMEINPUT_BUTTONS] = {
    SDL_BUTTON_LEFT,
    SDL_BUTTON_RIGHT,
    SDL_BUTTON_MIDDLE,
    SDL_BUTTON_X1,
...
typedef struct GAMEINPUT_Device
{
    ...
    char *name;
    uint32_t instance_id; // generated by SDL
    bool registered;
...
static void GAMEINPUT_InitialMouseReading(WIN_GameInputData *data, SDL_Window *window, GAMEINPUT_Device *device, IGameInputReading *reading)
{
    ...
    if (SUCCEEDED(IGameInputReading_GetMouseState(reading, &state))) {
        uint64_t timestamp = SDL_US_TO_NS(IGameInputReading_GetTimestamp(reading) + data->timestamp_offset);
...
{
    uint8_t index = (uint8_t)(state->scanCode & 0xFF);
