...
#define njAllocMem SDL_malloc
#define njFreeMem SDL_free
#define njFillMem SDL_memset
#define njCopyMem SDL_memcpy
...
void njConvert(bool convert_rgb) {
    int i;
    nj_component_t* c;
    for (i = 0, c = nj.comp;  i < nj.ncomp;  ++i, ++c) {
...
nj_result_t njDecode(const void* jpeg, const int size, bool convert_rgb) {
    njDone();
...
bool SDL_ConvertPixels_JPEG(int width, int height,
                           ...
                           SDL_PixelFormat dst_format, SDL_Colorspace dst_colorspace, SDL_PropertiesID dst_properties, void *dst, int dst_pitch)
{
	...
	/* decode image */
	len = (src_format == SDL_PIXELFORMAT_MJPG) ? src_pitch : (height * src_pitch);
    if (njDecode(src, len, true) != NJ_OK) {
        return false;
    }
	...
	h = njGetHeight();
    if (w == width && h == height) {
        result = SDL_ConvertPixelsAndColorspace(w, h, SDL_PIXELFORMAT_RGB24, SDL_COLORSPACE_SRGB, 0, pixels, width * 3, dst_format, dst_colorspace, dst_properties, dst, dst_pitch);
    } else {
