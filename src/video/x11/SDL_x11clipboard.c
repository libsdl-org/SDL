...
static int SetSelectionText(THIS, const char *text, Atom selection_type)
{
...
static char *GetSlectionText(THIS, Atom selection_type)
{
...
int X11_SetClipboardText(THIS, const char *text)
{
    SDL_VideoData *videodata = (SDL_VideoData *)_this->driverdata;
...
int X11_SetPrimarySelectionText(THIS, const char *text)
{
    return SetSelectionText(_this, text, XA_PRIMARY);
}

char *
X11_GetClipboardText(THIS)
{
    SDL_VideoData *videodata = (SDL_VideoData *)_this->driverdata;
    Atom XA_CLIPBOARD = X11_XInternAtom(videodata->display, "CLIPBOARD", 0);
    if (XA_CLIPBOARD == None) {
...
char *
X11_GetPrimarySelectionText(THIS)
{
    return GetSlectionText(_this, XA_PRIMARY);
}

SDL_bool
X11_HasClipboardText(THIS)
{
...
SDL_bool
X11_HasPrimarySelectionText(THIS)
{
    SDL_bool result = SDL_FALSE;
