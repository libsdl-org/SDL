...
static SDL_bool xsync_version_atleast(const int version, const int wantmajor, const int wantminor)
{
    return version >= ((wantmajor * 1000) + wantminor);
}

void X11_InitXsync(_THIS)
{
...
int X11_InitResizeSync(SDL_Window *window)
{
    ...
    Atom counter_prop = data->videodata->_NET_WM_SYNC_REQUEST_COUNTER;
    ...
    data->resize_id.lo = 0;
    data->resize_id.hi = 0;
    data->resize_in_progress = SDL_FALSE;
...
void X11_HandleConfigure(SDL_Window *window, XConfigureEvent *event)
{
    ...
    if (data->resize_id.lo || data->resize_id.hi) {
        data->resize_in_progress = SDL_TRUE;
    }
}
