...
static const struct xx_color_management_output_v4_listener xx_color_management_output_listener = {
...
static bool Wayland_add_display(SDL_VideoData *d, uint32_t id, uint32_t version)
{
    ...
    if (data->videodata->xx_color_manager_v4) {
        data->xx_color_management_output = xx_color_manager_v4_get_output(data->videodata->xx_color_manager_v4, output);
        xx_color_management_output_v4_add_listener(data->xx_color_management_output, &xx_color_management_output_listener, data);
        Wayland_GetOutputColorInfo(data);
    }
    return true;
}
...
static void Wayland_free_display(SDL_VideoDisplay *display, bool send_event)
{
    if (display) {
        ...
        if (display_data->xx_color_management_output) {
            xx_color_management_output_v4_destroy(display_data->xx_color_management_output);
        }
...
static void Wayland_InitColorManager(SDL_VideoData *d)
{
    for(int i = 0; i < d->output_count; ++i) {
        SDL_DisplayData *disp = d->output_list[i];
        disp->xx_color_management_output = xx_color_manager_v4_get_output(disp->videodata->xx_color_manager_v4, disp->output);
        xx_color_management_output_v4_add_listener(disp->xx_color_management_output, &xx_color_management_output_listener, disp);
                                  ...
                                  const char *interface, uint32_t version)
{
    ...
    if (SDL_strcmp(interface, "wl_compositor") == 0) {
        ...
        d->xx_color_manager_v4 = wl_registry_bind(d->registry, id, &xx_color_manager_v4_interface, 1);
...
static void Wayland_VideoCleanup(SDL_VideoDevice *_this)
{
    ...
    if (data->xx_color_manager_v4) {
        xx_color_manager_v4_destroy(data->xx_color_manager_v4);
        data->xx_color_manager_v4 = NULL;
    }
