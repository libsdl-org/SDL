...
void
ATOMIC_GLES_DefaultProfileConfig(_THIS, int *mask, int *major, int *minor)
{
...
int
ATOMIC_GLES_LoadLibrary(_THIS, const char *path) {
    /* Just pretend you do this here, but don't do it until ATOMIC_CreateWindow(),
       where we do the same library load we would normally do here
       because this gets called by SDL_CreateWindow() before ATOMIC_CreateWindow(),
...
void
ATOMIC_GLES_UnloadLibrary(_THIS) {
    /* As with ATOMIC_GLES_LoadLibrary(), we define our own "dummy" unloading function
       so we manually unload the library whenever we want. */
}

SDL_EGL_CreateContext_impl(ATOMIC)
...
/***********************************************************************************/
int
ATOMIC_GLES_SwapWindowFenced(_THIS, SDL_Window * window)
{
    ...
    windata->next_bo = ATOMIC_gbm_surface_lock_front_buffer(windata->gs);
    if (!windata->next_bo) {
        return SDL_SetError("Failed to lock frontbuffer");
    }
    fb = ATOMIC_FBFromBO(_this, windata->next_bo);
    if (!fb) {
    ...
    if (dispdata->modeset_pending) {
        ...
        ATOMIC_drmModeCreatePropertyBlob(viddata->drm_fd, &dispdata->mode, sizeof(dispdata->mode), &blob_id);
...
int
ATOMIC_GLES_SwapWindowDoubleBuffered(_THIS, SDL_Window * window)
{
    SDL_WindowData *windata = ((SDL_WindowData *) window->driverdata);
    SDL_DisplayData *dispdata = (SDL_DisplayData *) SDL_GetDisplayForWindow(window)->driverdata;
    ATOMIC_FBInfo *fb;
...
int
ATOMIC_GLES_SwapWindow(_THIS, SDL_Window * window)
{
    SDL_WindowData *windata = ((SDL_WindowData *) window->driverdata);
    ...
    if (windata->swap_window == NULL) {
        ...
        {
            windata->swap_window = ATOMIC_GLES_SwapWindowDoubleBuffered;
        } else {
            windata->swap_window = ATOMIC_GLES_SwapWindowFenced;
        }
    }
...
SDL_EGL_MakeCurrent_impl(ATOMIC)
