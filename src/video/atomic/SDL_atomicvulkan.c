...
#if SDL_VIDEO_VULKAN && SDL_VIDEO_DRIVER_ATOMIC
...
int ATOMIC_Vulkan_LoadLibrary(_THIS, const char *path)
{
...
void ATOMIC_Vulkan_UnloadLibrary(_THIS)
{
    if(_this->vulkan_config.loader_handle)
    {
        SDL_UnloadObject(_this->vulkan_config.loader_handle);
...
/* vkCreateInstance().                                               */
/*********************************************************************/
SDL_bool ATOMIC_Vulkan_GetInstanceExtensions(_THIS,
                                          SDL_Window *window,
                                          unsigned *count,
                                          const char **names)
{
    static const char *const extensionsForATOMIC[] = {
    ...
    return SDL_Vulkan_GetInstanceExtensions_Helper(
            count, names, SDL_arraysize(extensionsForATOMIC),
            extensionsForATOMIC);
}

void ATOMIC_Vulkan_GetDrawableSize(_THIS, SDL_Window *window, int *w, int *h)
{
    if (w) {
        *w = window->w;
    }
...
/* ATOMIC_Vulkan_GetInstanceExtensions(), like we do with              */
/* VK_KHR_DISPLAY_EXTENSION_NAME, which is what we need for x-less VK. */                
/***********************************************************************/
SDL_bool ATOMIC_Vulkan_CreateSurface(_THIS,
                                  SDL_Window *window,
                                  VkInstance instance,
                                  VkSurfaceKHR *surface)
{
    ...
    /* We don't receive a display index in ATOMIC_CreateDevice(), only
    ...
    */
    ...
    if(result != VK_SUCCESS)
    {
        SDL_SetError("vkCreateATOMICSurfaceKHR failed: %s",
