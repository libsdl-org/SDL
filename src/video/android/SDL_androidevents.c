...
void Android_OnPause(SDL_Window *window)
{
     ...
     */
#ifdef SDL_VIDEO_OPENGL_EGL
    if (window && (window->flags & SDL_WINDOW_OPENGL) && !window->external_graphics_context) {
...
void Android_OnResume(SDL_Window *window)
{
    ...
    if (window && (window->flags & SDL_WINDOW_OPENGL) && !window->external_graphics_context && !SDL_HasEvent(SDL_EVENT_QUIT)) {
...
bool Android_BlockEventLoop(void)
{
    return false;
}
