...
static int SDLCALL events_mainThreadCallbacks(void *arg)
{
...
#ifndef SDL_PLATFORM_EMSCRIPTEN /* Emscripten doesn't have threads */
    {
        ...
        SDLTest_AssertCheck(window != NULL, "Create window, expected non-NULL, got %p", (void*)window);
        ...
        SDL_WaitEvent(&event);
        SDLTest_AssertCheck(event.type == SDL_EVENT_USER, "Expected user event (0x%.4x), got 0x%.4x", SDL_EVENT_USER, event.type);
