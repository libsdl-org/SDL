...
static int TestVirtualJoystick(void *arg)
{
    ...
    SDLTest_AssertCheck(device_id > 0, "SDL_AttachVirtualJoystick() -> %d (expected > 0)", device_id);
    ...
    if (device_id > 0) {
        ...
        if (joystick) {
            ...
            }
            {
                ...
                SDLTest_AssertCheck(vendor_id == desc.vendor_id, "SDL_GetJoystickVendor() -> 0x%04" SDL_PRIx32 " (expected 0x%04" SDL_PRIx32 ")", vendor_id, desc.vendor_id);
            }
            {
                Uint16 product_id = SDL_GetJoystickProduct(joystick);
                SDLTest_AssertCheck(product_id == desc.product_id, "SDL_GetJoystickProduct() -> 0x%04" SDL_PRIx32 " (expected 0x%04" SDL_PRIx32 ")", product_id, desc.product_id);
            }
            {
                Uint16 product_version = SDL_GetJoystickProductVersion(joystick);
                SDLTest_AssertCheck(product_version == 0, "SDL_GetJoystickProductVersion() -> 0x%04" SDL_PRIx32 " (expected 0x%04" SDL_PRIx32 ")", product_version, 0);
            }
            {
                Uint16 firmware_Version = SDL_GetJoystickFirmwareVersion(joystick);
                SDLTest_AssertCheck(firmware_Version == 0, "SDL_GetJoystickFirmwareVersion() -> 0x%04" SDL_PRIx32 " (expected 0x%04" SDL_PRIx32 ")", firmware_Version, 0);
            }
            {
            ...
            }
            {
                ...
                SDLTest_AssertCheck(naxes == desc.naxes, "SDL_GetNumJoystickAxes() -> 0x%04" SDL_PRIx32 " (expected 0x%04" SDL_PRIx32 ")", naxes, desc.naxes);
            }
            {
            ...
            if (gamepad) {
                {
                    ...
                    SDLTest_AssertCheck(SDL_strcmp(name, desc.name) == 0, "SDL_GetGamepadName() -> \"%s\" (expected \"%s\")", name, desc.name);
                }
                {
                    Uint16 vendor_id = SDL_GetGamepadVendor(gamepad);
                    SDLTest_AssertCheck(vendor_id == desc.vendor_id, "SDL_GetGamepadVendor() -> 0x%04" SDL_PRIx32 " (expected 0x%04" SDL_PRIx32 ")", vendor_id, desc.vendor_id);
                }
                {
                    Uint16 product_id = SDL_GetGamepadProduct(gamepad);
                    SDLTest_AssertCheck(product_id == desc.product_id, "SDL_GetGamepadProduct() -> 0x%04" SDL_PRIx32 " (expected 0x%04" SDL_PRIx32 ")", product_id, desc.product_id);
                }
                ...
                {
                    const char *name = SDL_GetGamepadName(gamepad);
                    SDLTest_AssertCheck(SDL_strcmp(name, "Virtual Gamepad") == 0, "SDL_GetGamepadName() ->\"%s\" (expected \"%s\")", name, "Virtual Gamepad");
                }
                {
                ...
                {
                    const char *name = SDL_GetGamepadName(gamepad);
                    SDLTest_AssertCheck(SDL_strcmp(name, "Virtual Nintendo Gamepad") == 0, "SDL_GetGamepadName() -> \"%s\" (expected \"%s\")", name, "Virtual Nintendo Gamepad");
                }
