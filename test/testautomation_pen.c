...
_setup_test(pen_testdata *ptest, int pens_for_testing)
{
    ...
    for (i = 0; i < pens_for_testing; ++i) {
        ...
        SDLTest_AssertCheck(-1 == index,
                           "Registered PenID(%u) since index %d == -1", ptest->ids[i], index);
    }
...
_pen_dump(const char *prefix, SDL_Pen *pen)
{
    ...
    SDL_Log("%s: pen %u (%s): status=%04x, flags=%x, x,y=(%f, %f) axes = %s",
            prefix,
            pen->header.id, pen->name, pen->last.buttons,
...
_pen_simulate(simulated_pen_action *steps, int *step_counter, SDL_Pen *simulated_pens, int num_pens)
{
    ...
    do {
        ...
        case SIMPEN_ACTION_MOTION_EVENT:
            ...
            SDLTest_AssertCheck(SDL_SendPenMotion(simpen->header.id, SDL_TRUE,
                                                  &simpen->last),
        ...
        case SIMPEN_ACTION_PRESS:
            ...
            SDLTest_AssertCheck(SDL_SendPenButton(simpen->header.id, SDL_PRESSED, step.index),
        ...
        case SIMPEN_ACTION_RELEASE:
            mask = ~(1 << (step.index - 1));
            simpen->last.buttons &= mask;
            SDLTest_AssertCheck(SDL_SendPenButton(simpen->header.id, SDL_RELEASED, step.index),
    ...
    if (dump_pens) {
        ...
        for (i = 0; i < num_pens; ++i) {
            ...
            _pen_dump("actual", SDL_GetPen(simulated_pens[i].header.id));
        }
    }
...
_pen_simulate_init(pen_testdata *ptest, SDL_Pen *simulated_pens, int num_pens)
{
    int i;
    for (i = 0; i < num_pens; ++i) {
        simulated_pens[i] = *SDL_GetPen(ptest->ids[i]);
    }
}
...
pen_iteration(void *arg)
{
    ...
    for (i = 0; i < 3; ++i) {
        ...
        int index = _pen_iterationFindsPenIDAt(ptest.ids[i]);
        SDLTest_AssertCheck(-1 != index,  "Found PenID(%u)", ptest.ids[i]);
    }
...
{
    SDLTest_AssertCheck(-1 != _pen_iterationFindsPenIDAt(penid),  "Found PenID(%u)", penid);
    SDLTest_AssertCheck(SDL_PenAttached(penid), "Pen %u was attached, as expected", penid);
}

static void
_expect_pen_detached(SDL_PenID penid)
{
    SDLTest_AssertCheck(-1 == _pen_iterationFindsPenIDAt(penid),  "Did not find PenID(%u), as expected", penid);
    SDLTest_AssertCheck(!SDL_PenAttached(penid), "Pen %u was detached, as expected", penid);
}
...
//    SDL_PumpEvents();
    ...
    for (i = 0; i < 4; ++i) {
        SDLTest_AssertCheck(ptest.ids[i] == SDL_PenIDForGUID(ptest.guids[i]),
...
pen_buttonReporting(void *arg)
{
    ...
    for (pen_nr = 0; pen_nr < 2; ++pen_nr) {
        ...
        for (button_nr = 1; button_nr <= 8; ++button_nr) {
            ...
            SDL_SendPenButton(ptest.ids[pen_nr], SDL_PRESSED, button_nr);
    ...
    for (pen_nr = 0; pen_nr < 2; ++pen_nr) {
        ...
        for (button_nr = pen_nr + 1; button_nr <= 8; button_nr += 2) {
            ...
            SDL_SendPenButton(ptest.ids[pen_nr], SDL_RELEASED, button_nr);
            while (SDL_PollEvent(&event)) {
    ...
    while ((last_action = _pen_simulate(steps, &sim_pc, &simulated_pens[0], 2))) {
        ...
        if (reported_which != simpen->header.id) {
            ...
            SDLTest_AssertCheck(0, "Expected report for pen %u but got report for pen %u",
                                simpen->header.id, reported_which);
        }
        ...
        if (dump_pens) {
            ...
            _pen_dump("actual", SDL_GetPen(simpen->header.id));
        }
    }
                   ...
                   int expected_axes)
{
    ...
    SDLTest_AssertEq1(int, "%d", expected_attached, SDL_PenAttached(penid), "Pen %u is attached", penid);
    SDLTest_AssertEq1(int, "%d", expected_type, SDL_PenType(penid), "Pen %u type", penid);
    SDLTest_AssertEq1(int, "%x", expected_axes, SDL_PenCapabilities(penid, &actual_info), "Pen %u axis flags", penid);
    SDLTest_AssertEq1(int, "%d", expected_num_buttons, actual_info.num_buttons, "Pen %u number of buttons", penid);
    SDLTest_AssertEq1(float, "%f", expected_max_tilt, actual_info.max_tilt, "Pen %u max tilt", penid);
}
...
_penmouse_expect_button(int type, int button)
{
    ...
    SDLTest_AssertCheck((press ? SDL_MOUSEBUTTONDOWN : SDL_MOUSEBUTTONUP) == _mouseemu_last_event,
                        "Mouse button %s: %x",
...
pen_mouseEmulation(void *arg)
{
    ...
    for (i = 1; i <= 3; ++i) {
        SDL_SendPenButton(ptest.ids[0], SDL_PRESSED, i);
        _penmouse_expect_button(SDL_PRESSED, i);
    }
    SDLTest_AssertPass("Button press mouse emulation");

    /* Test button release reporting */
    for (i = 1; i <= 3; ++i) {
        SDL_SendPenButton(ptest.ids[0], SDL_RELEASED, i);
        _penmouse_expect_button(SDL_RELEASED, i);
    }
...
pen_mouseEmulationDelayed(void *arg)
{
    ...
    /* Touch surface (pen button 1) */
    SDL_SendPenButton(ptest.ids[0], SDL_PRESSED, 1);
    _penmouse_expect_button(SDL_PRESSED, 1);
    SDL_SendPenButton(ptest.ids[0], SDL_RELEASED, 1);
...
static const SDLTest_TestCaseReference penTest7 =
        { (SDLTest_TestCaseFp)pen_mouseEmulation, "pen_mouseEmulation", "Check pen-as-mouse event forwarding (direct)", TEST_ENABLED };

static const SDLTest_TestCaseReference penTest8 =
        { (SDLTest_TestCaseFp)pen_mouseEmulationDelayed, "pen_mouseEmulationDelayed", "Check pen-as-mouse event forwarding (delayed)", TEST_ENABLED };

/* Sequence of Mouse test cases */
static const SDLTest_TestCaseReference *penTests[] =  {
    &penTest1, &penTest2, &penTest3, &penTest4, &penTest5, &penTest6, &penTest7, &penTest8, NULL
};
