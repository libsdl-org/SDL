...
/* Note: This demo shows an example of multiview support using the SDL GPU API.
 * A minimum of two supported views are required, any less and the test will fail
 * to start.
 */
...
#include "testgpu/cube_multiview.vert.dxil.h"
#include "testgpu/cube_multiview.vert.msl.h"
#include "testgpu/cube_multiview.vert.spv.h"
...
static void Render(SDL_Window *window, const int windownum)
{
     ...
     */
    rotate_matrix(winstate->angle_x, 1.0f, 0.0f, 0.0f, matrix_modelview);
    rotate_matrix(winstate->angle_y, 0.0f, 1.0f, 0.0f, matrix_rotate);

    multiply_matrix(matrix_rotate, matrix_modelview, matrix_modelview);

    rotate_matrix(winstate->angle_z, 0.0f, 1.0f, 0.0f, matrix_rotate);
    ...
    /* Left view offset */
    matrix_modelview[12] = -0.1f;
    multiply_matrix(matrix_perspective, matrix_modelview, matrix_final.left);

    /* Right view offset */
    matrix_modelview[12] = 0.1f;
    multiply_matrix(matrix_perspective, matrix_modelview, matrix_final.right);

    winstate->angle_x += 0.3f;
    winstate->angle_y += 0.2f;
    winstate->angle_z += 0.1f;
    ...
    pass = SDL_BeginMultiViewGPURenderPass(cmd, &color_target, 1, &depth_target, 2);
    ...
    drawablew_half = drawablew / 2;

    /* Blit the two views side-by-side to the swapchain */
    for (i = 0; i < 2; i++) {
        blit_info.source.texture = winstate->tex_resolve;
        blit_info.source.w = drawablew_half;
        blit_info.source.h = drawableh;
        blit_info.source.layer_or_depth_plane = i;

        blit_info.destination.texture = swapchainTexture;
        blit_info.destination.w = drawablew_half;
        blit_info.destination.h = drawableh;
        blit_info.destination.x = drawablew_half * i;
...
static SDL_GPUShader *load_shader(bool is_vertex)
{
    ...
    if (format & SDL_GPU_SHADERFORMAT_DXIL) {
        ...
        createinfo.code = is_vertex ? cube_multiview_vert_dxil : cube_frag_dxil;
        createinfo.code_size = is_vertex ? cube_multiview_vert_dxil_len : cube_frag_dxil_len;
    } else if (format & SDL_GPU_SHADERFORMAT_MSL) {
        createinfo.format = SDL_GPU_SHADERFORMAT_MSL;
        createinfo.code = is_vertex ? cube_multiview_vert_msl : cube_frag_msl;
        createinfo.code_size = is_vertex ? cube_multiview_vert_msl_len : cube_frag_msl_len;
    } else {
        createinfo.format = SDL_GPU_SHADERFORMAT_SPIRV;
        createinfo.code = is_vertex ? cube_multiview_vert_spv : cube_frag_spv;
        createinfo.code_size = is_vertex ? cube_multiview_vert_spv_len : cube_frag_spv_len;
    }
...
static void init_render_state(int msaa)
{
    ...
    if (
        SDL_GetNumberProperty(
            SDL_GetGPUDeviceProperties(gpu_device),
            SDL_PROP_GPU_DEVICE_MAX_VIEW_COUNT_NUMBER,
            1
        ) < 2
    ) {
        SDL_Log("At least 2 views are required, which the GPU driver doesn't support");
        quit(2);
    }
