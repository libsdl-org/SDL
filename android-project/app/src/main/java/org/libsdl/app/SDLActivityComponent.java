...
*/
    ...
    // Main components
    protected static SDLActivityComponent mSingleton;
    ...
    public SDLActivityComponent(Activity activity) {
        if (mSingleton != null) {
            throw new IllegalStateException("Only one SDL activity component is allowed");
        }
        if (!(activity instanceof View.OnSystemUiVisibilityChangeListener)) {
            throw new IllegalStateException("Activity must implement View.OnSystemUiVisibilityChangeListener");
        }
        if (!(activity instanceof SDLComponentReceiver)) {
            throw new IllegalStateException("Activity must implement SDLComponentReceiver");
        }
     ...
     */
    protected void main() {
        String library = SDLActivityComponent.mSingleton.getMainSharedObject();
        String function = SDLActivityComponent.mSingleton.getMainFunction();
        String[] arguments = SDLActivityComponent.mSingleton.getArguments();

        Log.v("SDL", "Running main function " + function + " from library " + library);
        SDLActivityComponent.nativeRunMain(library, function, arguments);
    ...
    protected String getMainSharedObject() {
        String library;
        String[] libraries = SDLActivityComponent.mSingleton.getLibraries();
    ...
    // Load the .so
    public void loadLibraries(Activity activity) {
       for (String lib : getLibraries()) {
          SDL.loadLibrary(lib, activity);
       }
    }
    ...
    public void onCreate() {
        ...
        if (mSDLMainFinished || mActivityCreated) {
              boolean allow_recreate = SDLActivityComponent.nativeAllowRecreateActivity();
              if (mSDLMainFinished) {
        ...
        {
            int run_count = SDLActivityComponent.nativeCheckSDLThreadCounter(); /* get and increment a native counter */
            if (run_count != 0) {
                boolean allow_recreate = SDLActivityComponent.nativeAllowRecreateActivity();
                if (allow_recreate) {
        ...
        mSingleton = this;
        SDL.setContext(mActivity);

        mClipboardHandler = new SDLClipboardHandler();

        mHIDDeviceManager = HIDDeviceManager.acquire(mActivity);

        // Set up the surface
        mSurface = createSDLSurface(mActivity);

        mLayout = new RelativeLayout(mActivity);
        mLayout.addView(mSurface);

        // Get our current screen orientation and pass it down.
        SDLActivityComponent.nativeSetNaturalOrientation(SDLActivityComponent.getNaturalOrientation());
        mCurrentRotation = SDLActivityComponent.getCurrentRotation();
        SDLActivityComponent.onNativeRotationChanged(mCurrentRotation);
        ...
        case Configuration.UI_MODE_NIGHT_NO:
            SDLActivityComponent.onNativeDarkModeChanged(false);
            break;
        case Configuration.UI_MODE_NIGHT_YES:
            SDLActivityComponent.onNativeDarkModeChanged(true);
            break;
        }
        ...
        mActivity.getWindow().getDecorView().setOnSystemUiVisibilityChangeListener(
                (View.OnSystemUiVisibilityChangeListener) mActivity
        );
        ...
        if (intent != null && intent.getData() != null) {
            ...
            if (filename != null) {
                ...
                SDLActivityComponent.onNativeDropFile(filename);
            }
        }
    }
    ...
    protected void pauseNativeThread() {
        ...
        if (SDLActivityComponent.mBrokenLibraries) {
            return;
        }
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        if (hasFocus) {
           mNextNativeState = NativeState.RESUMED;
           SDLActivityComponent.getMotionListener().reclaimRelativeMouseModeIfNeeded();
    ...
    public void onTrimMemory(int level) {
        ...
        SDLActivityComponent.nativeLowMemory();
    }
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        if (mCurrentLocale == null || !mCurrentLocale.equals(newConfig.locale)) {
            ...
            SDLActivityComponent.onNativeLocaleChanged();
        }
    ...
    public void onDestroy() {
        ...
        SDLAudioManager.release(mActivity);

        if (SDLActivityComponent.mBrokenLibraries) {
           return;
        }

        if (SDLActivityComponent.mSDLThread != null) {

            // Send Quit event to "SDLThread" thread
            SDLActivityComponent.nativeSendQuit();
            ...
            try {
                ...
                SDLActivityComponent.mSDLThread.join(1000);
            } catch(Exception e) {
                Log.v(TAG, "Problem stopping SDLThread: " + e);
            }
        }

        SDLActivityComponent.nativeQuit();
    }

    /**
     * @return true if default system back button behaviour should be followed
     */
    @ActivityHook
    public boolean onBackPressed() {
        ...
        //
        boolean trapBack = SDLActivityComponent.nativeGetHintBoolean("SDL_ANDROID_TRAP_BACK_BUTTON", false);
        if (trapBack) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (mFileDialogState != null && mFileDialogState.requestCode == requestCode) {
            ...
            SDLActivityComponent.onNativeFileDialog(requestCode, filelist, -1);
    ...
    public static void manualBackButton() {
        // TODO: find a way to get the activity from JNA (you can't store it as static, memory leak)
        pressBackButton();
    }

    // Used to get us onto the activity's main thread
    private static void pressBackButton() {
        mSingleton.mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (!mSingleton.mActivity.isFinishing()) {
                    ((SDLComponentReceiver) mSingleton.mActivity).superOnBackPressed();
                }
            }
        });
    }
    ...
    /**
     * @return true if your activity should dispatch key events to superclass
     */
    @ActivityHook
    ...
    protected static class SDLCommandHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
                    ...
                    if (context instanceof Activity) {
                        ...
                        if (window != null) {
                            if ((msg.obj instanceof Integer) && ((Integer) msg.obj != 0)) {
                                ...
                                SDLActivityComponent.mFullscreenModeActive = true;
                            } else {
                                int flags = View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_VISIBLE;
                                window.getDecorView().setSystemUiVisibility(flags);
                                window.addFlags(WindowManager.LayoutParams.FLAG_FORCE_NOT_FULLSCREEN);
                                window.clearFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
                                SDLActivityComponent.mFullscreenModeActive = false;
                            }
                            ...
                            if (Build.VERSION.SDK_INT >= 30 /* Android 11 (R) */ &&
                                ...
                                SDLActivityComponent.onNativeInsetsChanged(0, 0, 0, 0);
                            }
                        }
                    } else {
                        Log.e(TAG, "error handling message, getContext() returned no Activity");
            ...
            default:
                if (context == mSingleton.mActivity && !mSingleton.onUnhandledMessage(msg.arg1, msg.obj)) {
    ...
    protected boolean sendCommand(int command, Object data) {
        ...
        if (Build.VERSION.SDK_INT >= 19 /* Android 4.4 (KITKAT) */) {
            if (command == COMMAND_CHANGE_WINDOW_STYLE) {
                ...
                if (bShouldWait && (SDLActivityComponent.getContext() != null)) {
                    ...
                    synchronized (SDLActivityComponent.getContext()) {
                        try {
                            SDLActivityComponent.getContext().wait(500);
    ...
    public void setOrientationBis(int w, int h, boolean resizable, String hint)
    {
        ...
        mActivity.setRequestedOrientation(req);
    }
    ...
    public static void minimizeWindow() {
        ...
        mSingleton.mActivity.startActivity(startMain);
    }
    ...
    public static boolean supportsRelativeMouse()
    {
        ...
        return SDLActivityComponent.getMotionListener().supportsRelativeMouse();
    }
    ...
    public static boolean setRelativeMouseEnabled(boolean enabled)
    {
        ...
        return SDLActivityComponent.getMotionListener().setRelativeMouseEnabled(enabled);
    }
...
//        }
        ...
        if ((source & InputDevice.SOURCE_MOUSE) == InputDevice.SOURCE_MOUSE) {
            if (SDLActivityComponent.isVRHeadset()) {
    ...
    public static Surface getNativeSurface() {
        if (SDLActivityComponent.mSurface == null) {
            return null;
        }
        return SDLActivityComponent.mSurface.getNativeSurface();
    }
    ...
    public void onSystemUiVisibilityChange(int visibility) {
        if (SDLActivityComponent.mFullscreenModeActive && ((visibility & View.SYSTEM_UI_FLAG_FULLSCREEN) == 0 || (visibility & View.SYSTEM_UI_FLAG_HIDE_NAVIGATION) == 0)) {
            ...
            Handler handler = mActivity.getWindow().getDecorView().getHandler();
            if (handler != null) {
    ...
    public static boolean openURL(String url)
    {
        try {
            ...
            mSingleton.mActivity.startActivity(i);
        } catch (Exception ex) {
            return false;
        }
        return true;
        ...
        try
        {
            class OneShotTask implements Runnable {
                ...
                public void run() {
                    try
                    {
                        Toast toast = Toast.makeText(mSingleton.mActivity, mMessage, mDuration);
                        if (mGravity >= 0) {
            ...
            mSingleton.mActivity.runOnUiThread(new OneShotTask(message, duration, gravity, xOffset, yOffset));
        } catch(Exception ex) {
            return false;
        }
        return true;
    ...
    public static int openFileDescriptor(String uri, String mode) throws Exception {
        ...
        try {
            ParcelFileDescriptor pfd = mSingleton.mActivity.getContentResolver().openFileDescriptor(Uri.parse(uri), mode);
    ...
    public static boolean showFileDialog(String[] filters, boolean allowMultiple, boolean forWrite, int requestCode) {
        ...
        try {
            mSingleton.mActivity.startActivityForResult(intent, requestCode);
...
class SDLMain implements Runnable {
    ...
    public void run() {
        ...
        SDLActivityComponent.nativeInitMainThread();
        SDLActivityComponent.mSingleton.main();
        SDLActivityComponent.nativeCleanupMainThread();

        if (SDLActivityComponent.mSingleton != null && !SDLActivityComponent.mSingleton.mActivity.isFinishing()) {
            // Let's finish the Activity
            SDLActivityComponent.mSDLThread = null;
            SDLActivityComponent.mSDLMainFinished = true;
            SDLActivityComponent.mSingleton.mActivity.finish();
...
class SDLClipboardHandler implements
    ...
    public void onPrimaryClipChanged() {
        SDLActivityComponent.onNativeClipboardChanged();
    }
}
