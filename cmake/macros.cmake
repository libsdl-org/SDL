...
function(read_absolute_symlink DEST PATH)
  ...
  if(NOT IS_ABSOLUTE "${target}")
    ...
    set(target "${pdir}/${target}")
  endif()
  get_filename_component(target "${target}" ABSOLUTE)
  set("${DEST}" "${target}" PARENT_SCOPE)
endfunction()
...
function(elf_read_soname SONAME LIBRARY)
  # Parse SONAME from library using `readelf`
  set(result "${SONAME}-NOTFOUND")
  if(EXISTS "${LIBRARY}")
  ...
  set("${SONAME}" "${result}" PARENT_SCOPE)
endfunction()

function(library_soname_from_symlink SONAME LIBRARY)
  ...
  set(result "${SONAME}-NOTFOUND")
  if(EXISTS "${LIBRARY}")
    get_filename_component(abs_library "${LIBRARY}" ABSOLUTE)
    while(IS_SYMLINK "${abs_library}")
      read_absolute_symlink(abs_library "${abs_library}")
    endwhile()
    if(APPLE)
    ...
    get_filename_component(libdir "${abs_library}" DIRECTORY)
    file(GLOB subfiles "${libdir}/*")
    set(similar_files "${abs_library}")
    foreach(subfile ${subfiles})
      if(IS_SYMLINK "${subfile}")
        ...
        if(subfile_target STREQUAL abs_library AND subfile MATCHES "${valid_shared_library_regex}")
    ...
    message(DEBUG "files that are similar to \"${abs_library}\": ${similar_files}")
    list(GET similar_files 0 item)
    get_filename_component(result "${item}" NAME)
  endif()
  message(DEBUG "library_soname_from_symlink(${LIBRARY}) -> ${result}")
  set(${SONAME} "${result}" PARENT_SCOPE)
endfunction()

function(library_soname_from_path DEST LIBRARY)
  set(result "${DEST}-NOTFOUND")
  if(WIN32)
    win32_implib_identify_dll(result "${LIBRARY}")
  else()
    # 1. On elf platforms, if readelf is available, parse the output of `readelf -d` and return the "(SONAME)" line
    # 2. Else:
    #     1. Find the target library a file might be symbolic linking to
    #     2. find all other files in the same folder that symbolic link to it
    #     3. sort all these files, and select the 1st item on Linux, and last on macOS
    if(UNIX AND NOT APPLE)
      elf_read_soname(result "${LIBRARY}")
    endif()
    if(NOT result)
      library_soname_from_symlink(result "${LIBRARY}")
    endif()
  endif()
  set("${DEST}" "${result}" PARENT_SCOPE)
endfunction()
  ...
  if(WIN32)
    ...
    if(UNIX AND NOT APPLE)
      foreach(location_property ${location_properties})
        ...
        if(library_path AND EXISTS "${library_path}")
          elf_read_soname(result "${library_path}")
          if(result)
            break()
          endif()
        endif()
    ...
    if(NOT result)
      foreach(location_property ${location_properties})
        get_target_property(library_path "${TARGET}" ${location_property})
        if(library_path AND EXISTS "${library_path}")
          library_soname_from_symlink(result "${library_path}")
