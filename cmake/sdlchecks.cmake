      ...
      if(LibUSB_FOUND)
        ...
        if(HAVE_LIBUSB_H)
          ...
          if(SDL_HIDAPI_LIBUSB_SHARED)
            if(NOT HAVE_SDL_LOADSO)
              message(WARNING "You must have SDL_LoadObject() support for dynamic hidapi libusb loading")
              set(HAVE_HIDAPI_LIBUSB_SHARED OFF)
            else()
...
macro(CheckLiburing)
  ...
  set(HAVE_LIBURING_SHARED OFF)
  set(HAVE_LIBURING OFF)
  if(SDL_LIBURING)
    find_path(LIBURING_H_INCLUDE_PATH NAMES liburing.h)
    if(LIBURING_H_INCLUDE_PATH)
      if(SDL_LIBURING_SHARED)
        if(NOT HAVE_SDL_LOADSO)
          message(WARNING "You must have SDL_LoadObject() support for dynamic liburing support")
        else()
          FindLibraryAndSONAME(uring-ffi)
          if(URING_FFI_LIB_SONAME)
            sdl_include_directories(PRIVATE SYSTEM ${LIBURING_H_INCLUDE_PATH})
            set(SDL_LIBURING_DYNAMIC "\"${URING_FFI_LIB_SONAME}\"")
            set(HAVE_LIBURING TRUE)
            set(HAVE_LIBURING_SHARED TRUE)
          endif()
        endif()
      endif()
      if(NOT HAVE_LIBURING)
        set(liburing_PKG_CONFIG_SPEC liburing)
        pkg_check_modules(PC_LIBURING IMPORTED_TARGET ${liburing_PKG_CONFIG_SPEC})
        if(PC_LIBURING_FOUND)
          set(HAVE_LIBURING TRUE)
          sdl_link_dependency(liburing LIBS PkgConfig::PC_LIBURING PKG_CONFIG_PREFIX PC_LIBURING PKG_CONFIG_SPECS ${liburing_PKG_CONFIG_SPEC})
        endif()
      endif()
    endif()
    if(HAVE_LIBURING)
